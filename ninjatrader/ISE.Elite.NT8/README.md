...
- Market, limit, and stop-market entry orders
...
```text
ISE.Elite.NT8.config.template
```

to:

```text
%USERPROFILE%\Documents\NinjaTrader 8\bin\Custom\ISE.Elite.NT8.config
```

Then verify the exact active contract:
...
Close NinjaTrader before replacing bridge DLLs. From a Visual Studio Developer PowerShell at the repository root:

```powershell
msbuild .\ninjatrader\ISE.Elite.NT8\ISE.Elite.NT8.csproj `
  /p:Configuration=Debug `
  /p:NinjaTraderInstallDir="C:\Program Files\NinjaTrader 8"
```

The post-build target copies the bridge and its ISE runtime dependencies to:

```text
%USERPROFILE%\Documents\NinjaTrader 8\bin\Custom
```

Restart NinjaTrader after each bridge rebuild.
...
1. Start NinjaTrader with a simulation-capable connection.
2. Verify `Sim101` is visible.
3. Verify the configured full contract exists and can be opened on a chart.
4. Confirm the bridge startup messages in Output Tab 1.
5. Confirm no order is submitted automatically.
6. Keep the Control Center Orders, Executions, and Positions tabs visible.
7. Use the later supervised smoke-test command to submit one micro contract.
8. Verify order ID correlation, execution quantity, fill price, and position state.
9. Cancel or flatten manually if any state becomes inconsistent.
10. Stop testing immediately if an event is not correlated.

## Current limitations

- The live event loop has not yet been connected to the Trading Brain.
- The bridge does not yet submit protective stop and target child orders.
...
Those items belong to the supervised paper-trading and hardening sprints. This package establishes the actual NT8 execution and event boundary without enabling autonomous trading.
