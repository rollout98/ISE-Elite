...
{
	public class IseEliteStrategySessionOptimizedFixed : Strategy
	{
		private double sma5;
		private double sma10;
		private bool hasPosition;
		private double entryFillPrice;  // ACTUAL fill price from broker
		...
		private Order entryOrder;  // Track the actual entry order

		protected override void OnStateChange()
		{
			if (State == State.SetDefaults)
			{
				Description = @"ISE Elite Automated Trading System - Session Optimized (Actual Fills)";
				Name = "IseEliteStrategySessionOptimizedFixed";
			...
			{
				Print("✅ ISE Elite Strategy LIVE - 1 AM CT Start, Actual Fill Tracking");
			}
		}
		...
		protected override void OnBarUpdate()
		{
			...
			{
				double priceChange = Close[0] - entryFillPrice;
			...
			if (hasPosition)
			{
				...
				if (priceChange >= 3.0)
				{
					...
					Print(string.Format("📈 ENTRY @{0:F2} EXIT @{1:F2} | Target Hit +3 | P&L ${2:F0} | Daily: ${3:F0}", 
						entryFillPrice, Close[0], pnl, dailyPnL));
					ExitLong();
					hasPosition = false;
					return;
				}
			...
			if (!hasPosition && tradesThisDay < 10)
			{
				...
				if (sma5 > sma10 && Close[0] > sma5)
				{
					...
					Print(string.Format("📈 ENTRY #{0} @{1:F2} | SMA5:{2:F2} > SMA10:{3:F2} | Contracts: {4} | Session: {5}", 
						tradesThisDay, Close[0], sma5, sma10, currentContracts, session));
					entryOrder = EnterLong(currentContracts, "Entry");
				}
			}
		...
		// CRITICAL: Track actual execution prices from broker
		protected override void OnOrderUpdate(Order order, double limitPrice, double stopPrice, int quantity, int filled, double averageFillPrice, OrderState orderState, DateTime time, ErrorCode errorCode, string ninjascriptName)
		{
			if (orderState == OrderState.Filled && order.Name == "Entry")
			{
				// ACTUAL entry fill price from broker
				entryFillPrice = averageFillPrice;
				hasPosition = true;
				Print(string.Format("✅ ENTRY FILLED @ {0:F2} (Actual Broker Fill)", averageFillPrice));
			}
			else if (orderState == OrderState.Filled && order.Name == "Sell")
			{
				// Exit happened - P&L already calculated in OnBarUpdate
				hasPosition = false;
				Print(string.Format("✅ EXIT FILLED @ {0:F2} (Actual Broker Fill)", averageFillPrice));
			}
		}
