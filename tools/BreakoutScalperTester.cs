...
{
    /// <summary>
    /// Breakout Scalper Backtest Tester
    /// </summary>
    public class BreakoutScalperTester
    {
        ...
        private BreakoutScalperSignal signal = new BreakoutScalperSignal();
        private List<TradeResult> trades = new List<TradeResult>();
        private double peakEquity = 0;
        private double currentEquity = 0;

        public BacktestResults RunBacktest(List<BreakoutScalperSignal.Bar> bars)
        {
            signal.Clear();
            trades.Clear();
            peakEquity = 0;
            ...
            for (int i = 5; i < bars.Count; i++)
            {
                ...
                if (activeTrade != null)
                {
                        ...
                        activeTrade.ExitPrice, activeTrade.EntryPrice, activeTrade.Direction))
                    {
                        ...
                        {
                            if (currentBar.Close >= (activeTrade.EntryPrice + 1.5))
        ...
        {
            Console.WriteLine("\n" + new string('=', 60));
            Console.WriteLine("BREAKOUT SCALPER BACKTEST RESULTS");
